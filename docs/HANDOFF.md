...
> Ultimo aggiornamento: settembre 2026 · deploy asset `?v=82` · SW `bsp-v82` · backend V4.14.
...
| `fallo-subito.js` | **flusso fallo a passi** nell'overlay destro (`#action-overlay`), con "← indietro" ad ogni step: `avviaFalloSubito` (PVL subisce), `apriTlAvversari`/`finalizzaFalloFatto` (PVL commette), overlay fallo avversario, tecnici, doppio/compensati; `ultimaAzioneEraCanestro(num)` (and-1). **Tecnico/Antisportivo/Panchina anche in "FALLO fatto" (v79)**: `apriTlAvversari(num)` aveva solo `Nessun/1/2/3 TL` (fallo personale) — aggiunti "Tecnico / Antisportivo →" (`faseSpecialeFalloFatto`/`faseCountFalloFattoSpeciale`, mirror di `faseSpecialeFalloSubito` ma pei TL agli avversari, valorizza `fallo_speciale`) e "Panchina / coach (fallo di squadra) →" (riusa `faseEsitoTecnicoNostro`/`registraTecnicoPanchinaNostra` già esistente, `num` ignorato di proposito — nessun giocatore attribuito). Fix collaterale: `ultimoFalloCorreggibile()` esclude ora `dettaglio === "TECNICO_PANCHINA"` (bug preesistente — riaprirlo per "correggi ultimo" lo confondeva con un fallo personale, `giocatore_num` vuoto). |
...
| `stats.js` | motore stat: `statsContesto`, `eventiPuliti` (dedup + drop ANNULLA/valido=FALSE), `calcolaBox`, `stintsDaEventi`, `calcolaAdvanced`. ⚠️ **Fix box "giocatore senza referto" (v82)**: un giocatore in campo (presente in `quintetto_mia`) ma senza MAI un evento individuale a suo nome (raro — es. pochi secondi di garbage time) restava fuori da `pg` per le gare **storiche** (`ctx.convocati` è sempre `[]` lì, a differenza del live dove i convocati precompilano `pg`) — spariva del tutto dal box invece di comparire con 0 a referto e i suoi minuti reali. Stesso identico problema era già stato rattoppato localmente altrove (`aggregaStagione` in `analisi.js`, `calcolaAIS` in `analisi-avanzata.js` — restano lì, ora sono no-op innocui) senza mai toccare la fonte: risolto una volta sola in `calcolaBox` (righe subito dopo `minutiDaStints`), beneficio automatico per ogni chiamante incluso `report-gara.js` (che l'ha fatto emergere) e la vista Stats storica/Segui Live. **periodo** (`boxPeriodo`, `periodiDisponibili_`, `punteggioPeriodo_` — Tot/Q1-Q4/1°T/2°T); render Stats (tabellino/andamento/tiri/**PBP**, `descriviEvento`, header compatto `scoreCompatto_`, barra controlli `barraControlliStats_`) e Adv (squadra/giocatori); **Segui Live** (`avviaModalitaSegui`, `pollSeguiLive` ~20s, timeout JSONP anti-freeze); `barraPunteggio`; fetch storico `scaricaEventiPartita` |
...
| `report-gara.js` | **Report gara (AI)** (sperimentale, `#view-report-gara`, riga in Altro → sezione "Debrief", sotto "Debrief possessi") — scegli una gara `Terminata` → un unico blocco di testo (prompt AI in cima + tutti i dati sotto), pronto da copiare in un assistente AI: box squadra (MIA/OPP) + Adv squadra (`calcolaAdvanced`, stesso motore della vista Adv) + box giocatori con eFG%/TS%/USG%/AST%/TOV%/Forz. (stesse formule di `vistaAnalisiGiocatori` in `analisi.js`, **replicate localmente** perché quella funzione produce HTML accoppiato a stato UI — non riusabile a valle come dato puro, stesso schema già seguito da `player-dev.js` per AIS) + play-by-play completo cronologico (`descriviEvento`, `stats.js`). Eventi della gara da `cacheEventiStagione` (stessa cache già condivisa in sola lettura da Rotazioni/Breakdown/Rating Net/Player Development — **mai** `scaricaEventiPartita`, che muterebbe `statsEventiRemoti`/`statsTargetId` della vista Stats live). Copia negli appunti con fallback riusando `copiaPromptFallback_` (`player-dev.js`). **Nessuna integrazione con `avversari.js`** (scelta esplicita): l'avversario nel testo è solo nome squadra + le sue statistiche di gara, già nel box squadra — niente note di scouting individuale. |
...
- **`view-report-gara`** — **Report gara (AI)** (sperimentale, da Altro → sezione "Debrief" → riga "Report gara (AI)"): back (→ Altro) + selettore gara (solo `Terminata`) + testo generato in una `<textarea readonly>` + "📋 Copia per AI". Vedi `js/report-gara.js`.
